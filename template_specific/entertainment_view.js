...
//    vat = $('#vat').val();
    ...
    $('#net_due').val(net_due);
    $('#net_due_show').val(net_due);
    $('#payment_amount_enter').val(net_due);
...
function add_staff() {
    ...
    if (typeof (form_submit_counter) == 'undefined') {
        ...
        $("#staff_hold").dialog("close");
        return; //order is saved to database, no further staff information modification is allowed.
    }
...
// Send form data to controller->model
    $.post("../sale/entertainment/save_order",
            {
